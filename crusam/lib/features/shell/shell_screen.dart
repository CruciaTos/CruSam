...
// ── Tree helpers ───────────────────────────────────────────────────────────
String? _findActive(List<_NavItem> items, String loc) {
  for (final item in items) {
    if (item is _Route && (loc == item.path || loc.startsWith('${item.path}/'))) return item.path;
    if (item is _Group) { final f = _findActive(item.children, loc); if (f != null) return f; }
  }
  return null;
}
...
bool _groupContainsActive(List<_NavItem> items, String active) {
  for (final item in items) {
    if (item is _Route && item.path == active) return true;
    if (item is _Group && _groupContainsActive(item.children, active)) return true;
  }
  return false;
}
...
class _ShellScreenState extends State<ShellScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final active = _findActive(_kNav, loc) ?? '/dashboard';
      ...
      body: Stack(children: [
        ...
        Positioned.fill(child: Container(
          ...
          child: ParticleNetwork(
            particleColor: Colors.white.withOpacity(0.60),
            lineColor: const Color(0xFF4F46E5).withOpacity(0.12),
            particleCount: 100, maxSpeed: 1.0, maxSize: 2.0,
            lineDistance: 100, drawNetwork: true,
            touchActivation: false, gravityType: GravityType.none, gravityStrength: 0.08,
          ),
        )),
        Row(children: [
          // ── Sidebar ──────────────────────────────────────────────────
          AnimatedContainer(
            ...
            child: ClipRect(
              child: _expanded
                  ? _ExpandedSidebar(
                      active: active, openGroups: _open,
                      onNavigate: (p) => context.go(p), onToggle: _toggle,
                    )
                  : _CollapsedSidebar(
                      active: active, onNavigate: (p) => context.go(p),
                    ),
            ),
          ),
          // ── Content ──────────────────────────────────────────────────
...
class _ExpandedSidebar extends StatelessWidget {
  ...
  const _ExpandedSidebar({
    required this.active, required this.openGroups,
  ...
  Widget build(BuildContext context) => Column(children: [
    ...
    Padding(
      ...
      child: _NavTile(icon: Icons.logout, label: 'Logout', selected: false, depth: 0, onTap: () {}),
    ),
  ]);
    ...
    for (final item in items) {
      if (item is _Route) {
        out.add(_NavTile(
          ...
          selected: active == item.path, depth: depth,
          onTap: () => onNavigate(item.path),
        ));
      } else if (item is _Group) {
        final isOpen   = openGroups.contains(item.label);
        final hasActive = _groupContainsActive(item.children, active);
        out.add(_GroupTile(
          icon: item.icon, label: item.label,
          isOpen: isOpen, hasActive: hasActive, depth: depth,
...
class _CollapsedSidebar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
      ...
      children: _kNav.map((item) {
        if (item is _Route) {
          return _CollapsedTile(
            icon: item.icon, label: item.label,
            selected: active == item.path, onTap: () => onNavigate(item.path),
          );
        } else if (item is _Group) {
          return _CollapsedGroupTile(group: item, active: active, onNavigate: onNavigate);
        }
    ...
    Padding(
      padding: const EdgeInsets.all(10),
      child: _CollapsedTile(icon: Icons.logout, label: 'Logout', selected: false, onTap: () {}),
    ),
  ]);
}

// ══════════════════════════════════════════════════════════════════════════
// _NavTile  (expanded – route leaf, any depth)
// ══════════════════════════════════════════════════════════════════════════
class _NavTile extends StatelessWidget {
  ...
  const _NavTile({
    ...
    required this.selected, required this.depth, required this.onTap,
  });
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            if (depth == 0) ...[
              Icon(icon, size: 18, color: selected ? Colors.white : AppColors.slate400),
              const SizedBox(width: 10),
            ] else
              Container(
                width: 1.5, height: 16,
                color: selected ? Colors.white.withOpacity(0.45) : AppColors.slate700,
            ...
            if (selected)
              Icon(Icons.chevron_right, size: 15, color: Colors.white.withOpacity(0.7)),
          ]),
        ),
      ),
    );
...
// ══════════════════════════════════════════════════════════════════════════
// _GroupTile  (expanded – collapsible group header, any depth)
// ══════════════════════════════════════════════════════════════════════════
class _GroupTile extends StatelessWidget {
  final IconData icon;
  final String label;
  final bool isOpen, hasActive;
  final int depth;
  final VoidCallback onTap;

  const _GroupTile({
    required this.icon, required this.label,
    required this.isOpen, required this.hasActive,
    required this.depth, required this.onTap,
  });
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          padding: EdgeInsets.only(left: 10 + indent, right: 10),
          decoration: BoxDecoration(
            color: (hasActive && depth == 0) ? AppColors.slate800 : Colors.transparent,
            borderRadius: BorderRadius.circular(8),
          ),
          child: Row(children: [
            if (depth == 0) ...[
...
// ══════════════════════════════════════════════════════════════════════════
// _CollapsedTile  (icon-only, with tooltip)
// ══════════════════════════════════════════════════════════════════════════
    ...
    child: Tooltip(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Center(
            child: Icon(icon, size: 19, color: selected ? Colors.white : AppColors.slate400),
          ),
        ),
      ),
    ),
...
// ══════════════════════════════════════════════════════════════════════════
// _CollapsedGroupTile  (icon + hover popup showing all sub-routes)
// ══════════════════════════════════════════════════════════════════════════
class _CollapsedGroupTile extends StatefulWidget {
  ...
  const _CollapsedGroupTile({
    required this.group, required this.active, required this.onNavigate,
  });
...
class _CollapsedGroupTileState extends State<_CollapsedGroupTile> {
  ...
  bool get _hasActive => _groupContainsActive(widget.group.children, widget.active);
    ...
    _entry = OverlayEntry(builder: (_) => Positioned(
      ...
      child: MouseRegion(
        ...
        child: _GroupPopup(
          ...
          onNavigate: (path) { widget.onNavigate(path); _remove(); },
        ),
      ),
    ));
  ...
  void _remove() { _entry?.remove(); _entry = null; }

  @override
  void dispose() { _timer?.cancel(); _remove(); super.dispose(); }
    ...
    child: Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Center(
            child: Icon(widget.group.icon, size: 19,
                color: _hasActive ? AppColors.indigo400 : AppColors.slate400),
          ),
        ),
      ),
    ),
...
// ══════════════════════════════════════════════════════════════════════════
// _GroupPopup  (the floating card shown in collapsed mode)
// ══════════════════════════════════════════════════════════════════════════
class _GroupPopup extends StatelessWidget {
  ...
  const _GroupPopup({required this.group, required this.active, required this.onNavigate});
  ...
  Widget build(BuildContext context) => Material(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.45), blurRadius: 18, offset: const Offset(3, 5))],
      ),
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                if (sel) const Icon(Icons.chevron_right, size: 14, color: Colors.white),
              ]),
            ),
          ),
        ));
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: Padding(
      ...
      child: Row(children: [
        ...
        if (expanded) ...[
          const SizedBox(width: 10),
          Expanded(child: Text('AARTI ENTERPRISES', style: AppTextStyles.sidebarBrand, overflow: TextOverflow.ellipsis)),
        ],
      ]),
    ),
  );
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      ...
      Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [
        Text('Admin User', style: AppTextStyles.bodyMedium.copyWith(color: Colors.white70)),
        Text('boridkar24@gmail.com', style: AppTextStyles.small.copyWith(color: Colors.white60)),
      ]),
