...
class TaxInvoicePreview extends StatelessWidget {
  ...
  Widget _divider(double t) =>
      Divider(color: _black, thickness: t, height: 6);
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Expanded(
                    child: Text(voucher.clientAddress, style: _body)),
  ...
  // ITEM TABLE
  // ══════════════════════════════════════════════════════════════════════════
  Widget _itemTable() => LayoutBuilder(builder: (ctx, bc) {
        final fixedWidth =
            _wSr + _wDateFr + _wDateTo + _wQty + _wRate + _wAmt;
        final descW =
            (bc.maxWidth - fixedWidth - 2.0).clamp(0.0, double.infinity);
        ...
        child: IntrinsicHeight(
          child: Row(
            ...
            children: [
              ...
              _dCell(
                  _fmtDate(voucher.rows.isNotEmpty
                      ? voucher.rows.first.fromDate
                      : ''),
                  _wDateFr,
                  center: true),
              _dCell(
                  _fmtDate(voucher.rows.isNotEmpty
                      ? voucher.rows.last.toDate
                      : ''),
                  _wDateTo,
                  center: true),
              Container(
                width: descW,
                decoration:
                    const BoxDecoration(border: Border(right: _bSide)),
                padding:
      ...
      child: Row(
        ...
        children: [
          ...
          Column(
            ...
            children: [
              _btaxRow(taxLabelW, 'Total amount before Tax',
                  voucher.baseTotal.toStringAsFixed(2)),
              _btaxRow(taxLabelW, 'Add : CGST 9%',
                  voucher.cgst.toStringAsFixed(2)),
              _btaxRow(taxLabelW, 'Add : SGST 9%',
                  voucher.sgst.toStringAsFixed(2)),
              _btaxRow(taxLabelW, 'Total Tax Amount',
              ...
              _btaxGrand(taxLabelW, 'Total Amount after Tax',
  ...
  Widget _btaxRow(double labelW, String label, String value,
      {bool bold = false}) =>
      Container(
        decoration: const BoxDecoration(
            border: Border(top: _bSideThin)),
        child: Row(children: [
          Container(
            width: labelW,
  ...
  Widget _btaxGrand(double labelW, String label, String value) =>
      Container(
        decoration: const BoxDecoration(
          color: _grandBg,
        ...
        child: Row(children: [
          Container(
            ...
            child: Text(
              ...
              textAlign: TextAlign.right,
              style:
                  _body.copyWith(fontSize: 10, fontWeight: FontWeight.w900),
            ),
          ),
          SizedBox(
            width: _wAmt,
  ...
  Widget _declarationRow() => Container(
        ...
        child: Text(
          ...
          style:
              _body.copyWith(fontStyle: FontStyle.italic, fontSize: 8.5),
        ),
      );

  // ── Table cell builders ────────────────────────────────────────────────────
  Widget _hCell(String t, double w,
      {bool center = false, bool isLast = false}) =>
      Container(
        width: w,
        decoration: BoxDecoration(
  ...
  Widget _dCell(String t, double w,
      {bool center = false,
      bool right = false,
      bool bold = false,
      bool isLast = false}) =>
