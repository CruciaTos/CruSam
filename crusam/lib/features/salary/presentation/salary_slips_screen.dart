...
class _SalarySlipsScreenState extends State<SalarySlipsScreen> {
  ...
  void initState() {
    super.initState();
    // Employees already loaded by SalaryEmployeesScreen via controller.
    // If navigating directly, load them.
  ...
  List<String> get _codes => _stateCtrl.companyCodes;
...
class _CodeFilter extends StatelessWidget {
  ...
  static Widget _chip(String label, String? value, String selected, void Function(String?) onTap) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: active ? AppColors.indigo600 : AppColors.slate300),
          borderRadius: BorderRadius.circular(20),
          boxShadow: active ? [BoxShadow(color: AppColors.indigo600.withOpacity(0.28), blurRadius: 8, offset: const Offset(0, 2))] : null,
        ),
        child: Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
            color: active ? Colors.white : AppColors.slate600, letterSpacing: 0.2)),
      ),
    );
  }
}
...
class _EmployeePanel extends StatelessWidget {
  ...
  const _EmployeePanel({required this.employees, required this.selectedEmployee,
      required this.searchCtrl, required this.searchQuery, required this.onSearchChanged,
      required this.onSearchCleared, required this.onSelect, required this.getDays});
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      child: SizedBox(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Search employees…', prefixIcon: const Icon(Icons.search, size: 16), isDense: true,
            suffixIcon: searchQuery.isNotEmpty ? IconButton(icon: const Icon(Icons.clear, size: 14), onPressed: onSearchCleared) : null,
          ),
        ),
      ),
    ),
    Padding(
      ...
      child: Text('${employees.length} employee${employees.length == 1 ? '' : 's'}', style: AppTextStyles.small),
    ),
...
class _EmpTile extends StatelessWidget {
  final EmployeeModel employee; final bool isSelected; final int days; final VoidCallback onTap;
  const _EmpTile({required this.employee, required this.isSelected, required this.days, required this.onTap});
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: isSelected ? AppColors.indigo500.withOpacity(0.35) : Colors.transparent),
      ),
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
          Text(employee.name, style: AppTextStyles.body.copyWith(fontSize: 13,
        ...
        if (days > 0)
          Container(
            ...
            child: Text('${days}d', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.emerald700)),
          ),
      ]),
    ),
  );
...
class _PreviewPanel extends StatelessWidget {
  final EmployeeModel employee; final CompanyConfigModel config;
  final _SlipCalc calc; final String month, year, department;
        ...
        child: Center(
          child: ConstrainedBox(
            ...
            child: SalarySlipPreview(
              config: config, employeeName: employee.name, employeeCode: employee.code,
              designation: 'Technician', department: department,
