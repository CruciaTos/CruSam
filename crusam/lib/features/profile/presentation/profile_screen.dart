...
// ── Export Paths Card ──────────────────────────────────────────────────────────
//
// Shows two path-picker rows — one for PDF exports and one for Excel exports.
// On desktop (Windows / macOS / Linux) tapping the folder icon opens a native
// directory picker via `file_selector`.  On mobile the field is editable so
// users can paste a path, but the folder icon is hidden since mobile sandboxing
// makes custom directory selection impractical (files go through share_plus).

class _ExportPathsCard extends StatefulWidget {
  const _ExportPathsCard();
  @override
  State<_ExportPathsCard> createState() => _ExportPathsCardState();
}

class _ExportPathsCardState extends State<_ExportPathsCard> {
  ...
  void _onPrefsChanged() {
    if (_pdfCtrl.text != _prefs.pdfPath) {
      _pdfCtrl.text = _prefs.pdfPath;
    }
  ...
  Future<void> _pickPdfDirectory() async {
    final path = await _openDirectoryPicker('Choose PDF Export Folder');
    if (path == null || !mounted) return;
    _pdfCtrl.text = path;
    await _savePdf();
  }

  Future<void> _pickExcelDirectory() async {
    final path = await _openDirectoryPicker('Choose Excel Export Folder');
    if (path == null || !mounted) return;
  ...
  /// cancels or if the platform does not support it.
  Future<String?> _openDirectoryPicker(String confirmButtonText) async {
    try {
      final result = await getDirectoryPath(confirmButtonText: confirmButtonText);
      return result;
    } catch (_) {
      return null;
    }
  ...
  Future<void> _savePdf() async {
    ...
    _showSnack('PDF export path saved.');
  }
  ...
  Future<void> _clearPdf() async {
    ...
    if (!mounted) return;
    _showSnack('PDF path reset to default.');
  }
...
// ── _PathRow ───────────────────────────────────────────────────────────────────
//
// Reusable row that shows a labelled text field with a folder-picker button
// (desktop only), a Save button, and a clear/reset icon.
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
          ...
          children: [
            ...
            Expanded(
              child: TextField(
                ...
                style: AppTextStyles.input,
                readOnly: !supportsDirectoryPicker && Platform.isAndroid ||
                    !supportsDirectoryPicker && Platform.isIOS,
                decoration: InputDecoration(
                  hintText: hasPath
                      ? currentSavedPath
            ...
            // Clear / reset icon — only shown when a custom path is saved
            if (hasPath) ...[
        ...
        // Current saved path label (shown only when set)
        if (hasPath)
          Padding(
